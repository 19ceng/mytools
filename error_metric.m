function em = error_metric(y, yh, class_specific)
% function em = error_metric(y, yh, class_specific)
%
% IN:
%   y: actual value
%   yh: predicted value
%   class_specific: Sinif bilgisinin oldugu durumlari idare et (default:
%   false)
%
% OUT:
%   em: Error Metric Struture
%
% Usage:
%   em = error_metric(y, yh);
%   em.rms: RMS error
%
% Author: seyyah@bil.omu.edu.tr

if nargin == 2, class_specific = false; end

%% General
e = y - yh;
em.mse = sum(e.^ 2) / length(y);
em.rms = sqrt(em.mse); % => RMSE (E: error), RMSD (D: deviation)
em.mae = mean(abs(e));
em.me  = mean(e);
em.std = std(e);
em.median = median(abs(e));
em.min = min(e);
em.max = max(e);
em.rmsd = sqrt(em.mse / sum(y .^ 2));       % RMS Difference

% Tek sinif ozel durumunu idare et
mx = max(y);    mn = min(y);
if mx == mn,
    f = 1;
else
...
em.nrms = em.rms / f;       % Normalized RMS

%% Correlation
N = length(y);
sXY = sum(y .* yh);
sX = sum(y);        sY = sum(yh);
sX2= sum(y .^ 2);   sY2= sum(yh .^ 2);
% Tek sinif ozel durumunu idare et
mx = max(y);    mn = min(y);
if mx == mn,
    f = 1;
...
em.corr = (N * sXY - sX * sY) / sqrt(f * (N * sY2 - sY^2));

r = regression(y', yh');
em.Rkare = r(1);

%% errperf
em.mare = errperf(y, yh, 'mare');
em.msre = errperf(y, yh, 'msre');
em.rmsre = errperf(y, yh, 'rmsre');

em.mape = errperf(y, yh, 'mape');
em.mspe = errperf(y, yh, 'mspe');
em.rmspe = errperf(y, yh, 'rmspe');

%% Sinifa ozel basarim olcutleri
% y degerlerinin hepsi tamsayiysa
if class_specific && sum(round(y) - y) == 0
    c = unique(round(y));

    for i=1:length(c)
        ind = find(y == c(i));
        em.class_specific(i) = error_metric(y(ind), yh(ind));
    end

    % em.classperf = classperf(y, round(yh));
end
